...
class SemesterTableCellRenderer(var defaultRenderer: TableCellRenderer) : TableCellRenderer {
    override fun getTableCellRendererComponent(table: JTable, value: Any, isSelected: Boolean, hasFocus: Boolean, row: Int, column: Int): Component {
        val c = defaultRenderer.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column)
        if ((table.model as SemesterTable).getSemester().getSubject(row).isFinalized) {
            c.background = Window.Companion.DONE_COLOR
        } else {
